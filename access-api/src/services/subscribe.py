...
async def fetch_price(db: Session, subscribe_id: uuid.UUID):
    output = await db.query(Subscription).filter(Subscription.id == subscribe_id).one()
    return output.all_time_cost


async def get_price_id(db: Session, subscribe_id: uuid.UUID) -> str:
    """
    ...
    """
    return await db.query(Subscription).filter(Subscription.id == subscribe_id).one().payment_gw_price_id
...
async def grant_access(db: Session, grant_create: SimpleGrantAccessCreate):
    ...
    :return: ORM object
    """
    subscription = await db.query(Subscription).filter(Subscription.id == grant_create.subscription_id).one()
...
async def read_movie_access(db: Session, user_uuid: uuid.UUID, movie_uuid: uuid.UUID) -> bool:
    ...
    :return: yes or not
    """
    query = await db.query(GrantedFilms).filter(
...
async def check_active_days_left(db: Session, grant_access_id: uuid.UUID):
    end_date = await db.query(GrantedAccess).filter(GrantedAccess.uuid == grant_access_id).one().available_until
    delta = end_date - datetime.datetime.now()
    return delta.days


async def get_refund_amount_subscribe(db: Session, grant_access_id: uuid.UUID, days: int):
    subscription_id = await db.query(GrantedAccess).filter(GrantedAccess.uuid == grant_access_id).one().subscription_id
    price_per_day = await db.query(Subscription).filter(Subscription.id == subscription_id).one().cost
