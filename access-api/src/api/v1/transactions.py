...
from services import transactions_cr, utils
...
@router.get('/{user_uuid}/pagination', response_model=list[Transaction], dependencies=[Depends(utils.check_auth_key)])
...
@router.get('/{user_uuid}', response_model=list[Transaction], dependencies=[Depends(utils.check_auth_key)])
