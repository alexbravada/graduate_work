...
from services import balance_check, subscribe, transactions_cr, utils
...
@router.post('/request', dependencies=[Depends(utils.check_auth_key)])
...
@router.get('/grant/{grant_id}/', dependencies=[Depends(utils.check_auth_key)])
async def refund_subscribe(grant_id: uuid.UUID, db: Session = Depends(get_db)):
    ...
    grant = await db.query(GrantedAccess).filter(GrantedAccess.uuid == grant_id).one()
    ...
    try:
        ...
        await db.query(GrantedAccess).filter(GrantedAccess.uuid == grant_id).update({
            'available_until': datetime.datetime.now(),
            'is_active': False
        })

        films = await db.query(GrantedAccess).filter(GrantedAccess.uuid == grant_id).one().films
        for film in films:
        ...
        await db.query(TransactionOrder).filter(TransactionOrder.granted_access_id == grant_id).update(
...
@router.get('/transaction/{transaction_id}', dependencies=[Depends(utils.check_auth_key)])
async def refund_transaction(transaction_id: uuid.UUID, db: Session = Depends(get_db)):
    ...
    transaction = await db.query(Transaction).filter(Transaction.uuid == transaction_id).one()
    ...
    try:
        ...
        await db.query(TransactionOrder).filter(TransactionOrder.uuid == transaction.topup_order[0].uuid).update(
